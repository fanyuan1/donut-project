(function() {
    ...
    var squareframe=function() {
        ...
        var cA=Math.cos(A), sA=Math.sin(A),
            cB=Math.cos(B), sB=Math.sin(B);
        for(var i=square_len/-2;i<=square_len/2;i+=square_stepsize) { // j
            for (var j=square_len/-2;j<=square_len/2;j+=square_stepsize) {
                for (var k=square_len/-2;k<=square_len/2;k+=square_stepsize) {

                    var ox = i;
                    var oy = j;
                    var oz = k;

                    if([square_len/-2,square_len/2].indexOf(ox) == -1 &&
                    [square_len/-2,square_len/2].indexOf(oy) == -1 &&
                    [square_len/-2,square_len/2].indexOf(oz) == -1) {
                        continue;
                    }
                    ...
                    var xp=(150+square_K1*ooz*x);
                    var yp=(120-square_K1*ooz*y);

                    if([square_len/-2,square_len/2].indexOf(ox) == -1) {ox = 0;}
                    if([square_len/-2,square_len/2].indexOf(oy) == -1) {oy = 0;}
                    if([square_len/-2,square_len/2].indexOf(oz) == -1) {oz = 0;}

                    var L=0.7*(ox*-1*(cA*sB+sA*sB) + oy*(cA*cB+sA*cB) + oz*(sA-cA));
                    if(L > 0) {
                        ctx.fillStyle = 'rgba(255,255,255,'+L+')';
                        ctx.fillRect(xp, yp, 1.5, 1.5);
                    }
    ...
    window.anim3 = function() {
        if(tmr2 === undefined) {
        tmr2 = setInterval(squareframe, 50);
        } else {
        clearInterval(tmr2);
        tmr2 = undefined;
        }
